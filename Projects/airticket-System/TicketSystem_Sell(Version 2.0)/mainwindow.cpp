...
MainWindow::MainWindow(QWidget *parent)
   ...
   QPalette palette;
   QPixmap pixmap(":/source/images/3.jpg");
...
void MainWindow::enable_button(bool boolean){
   ...
   //ui->action_connect->setEnabled(false);    //将菜单栏设置为灰色不可用
...
void MainWindow::on_action_about_triggered(){
    ...
    form.addRow(new QLabel("作者信息: 崔凤朝 梁世豪 罗晓萌"));
    form.addRow(new QLabel("更新日期: 2020年06月18日"));
    QDialogButtonBox buttonBox(QDialogButtonBox::Ok, &dialog);
    form.addRow(&buttonBox);
    QObject::connect(&buttonBox, SIGNAL(accepted()), &dialog, SLOT(accept()));
    if (dialog.exec() == QDialog::Accepted) {
...
void MainWindow::on_action_add_triggered()
{
    ...
    QDialog dialog(this);
    QFormLayout form(&dialog);
    dialog.setWindowTitle("增加机票信息");
    QList<QLineEdit *> fields;
    QLineEdit *id = new QLineEdit(&dialog);
    form.addRow(new QLabel("请输入该机票的航班号:"));
    form.addRow(id);
    QLineEdit *number = new QLineEdit(&dialog);
    form.addRow(new QLabel("请输入该机票的票数:"));
    form.addRow(number);
    QLineEdit *price = new QLineEdit(&dialog);
    form.addRow(new QLabel("请输入该机票的票价:"));
    form.addRow(price);
    fields << id;
    fields << number;
    fields << price;
